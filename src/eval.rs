        ...
        match *self {
            ...
            Expr::Sym(ref symbol) => {
                env.lookup(&symbol.0).ok_or_else(||
                    Error::Msg(format!("undefined symbol: {}", symbol.0))
                )
            }
            _ => Ok(self.clone()),
        }
...
impl List {
    pub fn eval(&self, env: Env) -> Result<Expr> {
        if let Some((first, rest)) = self.0.split_first() {
            let sym = first.sym().ok_or(Error::Msg(format!("expected function call")))?;
            ...
            if forms::is_special_form(sym) {
                ...
                Err(Error::Msg(format!("could not find symbol {}", first)))
            }
        ...
        match *self {
            ...
            Function::User { ref name, ref params, ref body, ref env } => {
                ...
                if args.len() != params.len() {
                    return Err(Error::Msg(format!("fn expected {} args", params.len())));
                }
